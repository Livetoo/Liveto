...
describe('WalletStandardProvider specific tests', () => {
    ...
    it('should call signAndSendTransaction with correct params', async () => {
        ...
        expect(wallet.features[SolanaSignAndSendTransaction].signAndSendTransaction).toHaveBeenCalledWith({
            transaction: transaction.serialize({ verifySignatures: false }),
            account: wallet.accounts[0],
            chain: 'solana:mainnet',
    ...
    it('should throw if features are not available', async () => {
        wallet.features = {};
        await expect(walletStandardProvider.connect()).rejects.toThrowError(WalletStandardFeatureNotSupportedError);
        await expect(walletStandardProvider.disconnect()).rejects.toThrowError(WalletStandardFeatureNotSupportedError);
        await expect(walletStandardProvider.signTransaction(mockLegacyTransaction())).rejects.toThrowError(WalletStandardFeatureNotSupportedError);
        await expect(walletStandardProvider.signMessage(new Uint8Array())).rejects.toThrowError(WalletStandardFeatureNotSupportedError);
        await expect(walletStandardProvider.signAndSendTransaction(mockLegacyTransaction())).rejects.toThrowError(WalletStandardFeatureNotSupportedError);
    });
});
//# sourceMappingURL=WalletStandardProvider.test.js.map